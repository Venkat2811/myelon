...
fn producer_process() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Trigger consumer discovery so backpressure works.
    // The consumer has attached (coordination confirmed), now the producer's
    // barrier needs to find the consumer's cursor.
...
fn consumer_process() -> Result<(), Box<dyn std::error::Error>> {
    let (codec, _batch_size, _messages, segment) = read_codec_env();
...
fn run_codec_bench(codec: &str, batch_size: usize, messages: u64) -> BenchResult {
    ...
    let producer = spawn_child(&exe, "codec_producer", &segment, codec, batch_size, messages);
    let consumer = spawn_child(&exe, "codec_consumer", &segment, codec, batch_size, messages);
    ...
    let prod_str = match prod_out {
        Ok(o) => {
            if !o.stderr.is_empty() {
                eprintln!("Prod stderr [{codec}/{batch_size}]: {}", String::from_utf8_lossy(&o.stderr));
            }
    ...
    let cons_str = match cons_out {
        Ok(o) => {
            if !o.stderr.is_empty() {
                eprintln!("Cons stderr [{codec}/{batch_size}]: {}", String::from_utf8_lossy(&o.stderr));
            }
            String::from_utf8_lossy(&o.stdout).to_string()
        }
        Err(e) => {
