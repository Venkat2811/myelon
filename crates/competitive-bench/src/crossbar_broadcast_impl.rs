...
pub fn validate_args(args: &Args) -> Result<(), Box<dyn std::error::Error>> {
    if args.message_size < HEADER_BYTES {
        return Err(format!("message_size {} is too small; need at least {HEADER_BYTES} bytes", args.message_size).into());
    }
    if args.consumers == 0 {
...
fn consumer_result_path(base: &str, consumer_id: usize) -> PathBuf {
    std::env::temp_dir().join(format!("{base}_crossbar_broadcast_consumer_{consumer_id}.json"))
}
...
fn ring_depth_for(total_messages: u64, consumers: usize) -> u32 {
    let needed = (total_messages as usize).max(consumers.saturating_mul(32)).max(8);
...
fn load_consumer_results(base: &str, consumers: usize) -> Result<Vec<ConsumerResult>, Box<dyn std::error::Error>> {
...
fn worst_consumer_latency(results: &[ConsumerResult]) -> Result<LatencyStatsOut, Box<dyn std::error::Error>> {
    results
        .iter()
...
fn emit_result(
    ...
    let result = BenchmarkResultsOut {
        ...
        fanout_throughput: Some((args.num_messages * args.consumers as u64) as f64 / total_duration.as_secs_f64()),
...
fn publish_with_optional_rate<F>(args: &Args, mut publish: F) -> Result<Duration, Box<dyn std::error::Error>>
where
...
pub fn controller(args: Args) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if !coord.wait_for_consumers(args.consumers, COORD_TIMEOUT) {
        return Err(format!("timed out waiting for {} consumers to subscribe", args.consumers).into());
    }
    ...
    let publish_duration = publish_with_optional_rate(&args, |measured_idx, timestamp_ns, intended_send_time_ns| {
    ...
    if !coord.wait_for_consumers_done(args.consumers, COORD_TIMEOUT) {
        return Err(format!("timed out waiting for {} consumers to finish", args.consumers).into());
    }
