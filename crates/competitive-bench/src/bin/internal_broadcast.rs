...
use disruptor_mp::{
    ...
    build_shared_single_producer as disruptor_build_shared_single_producer,
    AutoWaitStrategy, MmapConsumer, MmapProducer, MmapTransportLayout,
...
fn controller<const SIZE: usize>(
    ...
    let buffer_size = args
        .buffer_size
        .unwrap_or_else(|| effective_buffer_size(args.message_size, args.consumers, args.num_messages));
...
fn run_controller_shm<const SIZE: usize>(
    ...
    debug_log!("shm controller: coordination reports {} consumers ready", args.consumers);
    ...
    debug_log!("shm controller: discovered all consumers: {:?}", consumer_names);
    ...
    let publish_duration = publish_with_optional_rate::<SIZE, _>(args, |sequence, timestamp_ns, intended_send_time_ns| {
        producer.publish(|slot| write_event(slot, sequence, timestamp_ns, intended_send_time_ns));
    });
    coord.signal_producer_done(args.num_messages as i64);
    debug_log!("shm controller: measured publish complete ({})", args.num_messages);
...
fn run_controller_mmap<const SIZE: usize>(
    ...
    let result = (|| {
        ...
        if !producer.wait_for_consumers_ready(args.consumers as i64, COORD_TIMEOUT) {
            return Err::<(), Box<dyn std::error::Error>>("timeout waiting for mmap broadcast consumers".into());
        }
        if !coord.wait_for_consumers(args.consumers, COORD_TIMEOUT) {
            return Err::<(), Box<dyn std::error::Error>>("timeout waiting for mmap coordination consumers".into());
        }
        ...
        if !coord.wait_for_consumers_done(args.consumers, COORD_TIMEOUT) {
            return Err::<(), Box<dyn std::error::Error>>("timeout waiting for mmap broadcast consumers to finish".into());
        }
        let total_duration = total_start.elapsed();
        let results = load_consumer_results(&args.base, args.consumers)?;
        let verification_passed = results.iter().all(|r| r.verification_passed);
...
fn consumer<const SIZE: usize>(args: Args, adapter: AdapterKind) -> Result<(), Box<dyn std::error::Error>> {
    let consumer_id = args.consumer_id.ok_or("--consumer-id is required")?;
    let result_path = args.result_path.clone().ok_or("--result-path is required")?;
    let buffer_size = args
        .buffer_size
        .unwrap_or_else(|| effective_buffer_size(args.message_size, args.consumers, args.num_messages));
    let coord = BenchmarkCoordination::attach_with_timeout(&coordination_prefix(&args.base), COORD_TIMEOUT)?;
...
fn consume_shm<const SIZE: usize>(
    ...
    debug_log!("shm consumer {consumer_id}: warmup complete ({})", args.warmup);
...
fn consume_mmap<const SIZE: usize>(
    ...
    let mut consumer = loop {
        match MmapConsumer::<BenchmarkEvent<SIZE>>::attach(layout.clone(), buffer_size, &consumer_name) {
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if args.mode == "controller" {
        run_with_large_stack_if_needed(args.message_size, "internal-broadcast-controller", move || {
            dispatch_by_size!(args.message_size, controller, args, adapter)
        })
    } else {
        run_with_large_stack_if_needed(args.message_size, "internal-broadcast-consumer", move || {
            dispatch_by_size!(args.message_size, consumer, args, adapter)
        })
    }
}
