...
use myelon::{MmapConsumer, MmapProducer};
...
fn producer_process() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut producer =
        MmapProducer::<Event>::create(layout, MYELON_BENCH_BUFFER_SIZE, Event::default)?;
...
fn consumer_process() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut consumer = loop {
        match MmapConsumer::<Event>::attach(
            layout.clone(),
    ...
    while consumed < num_events {
        if let Some(event) = consumer.try_consume_next_leased() {
            ...
            infra::check_deadline(deadline, "wait_strategy_myelon_mmap consumer_process");
...
impl IpcBenchmark for Scenario {
    ...
    fn launch(&self, exe: &std::path::Path) -> Result<ScenarioChildren, infra::BenchError> {
        let root = unique_mmap_root("mywait_mmap");
...
const CHILD_ROLES: &[infra::ChildRole] = &[
    infra::ChildRole::new("my_mmap_wait_producer", producer_process),
    infra::ChildRole::new("my_mmap_wait_consumer", consumer_process),
];

pub struct WaitStrategyMyelonMmapBench;

impl infra::BenchHarness for WaitStrategyMyelonMmapBench {
    ...
    fn run_orchestrator(&self, args: &[String]) -> infra::BenchRunResult {
        let mode_owned = env::var(crate::infra::env::MODE)
            ...
            .or_else(|| {
                args.iter()
                    .skip(1)
                    .find(|arg| {
                        !arg.starts_with("--") && arg.as_str() != "wait_strategy_myelon_mmap"
                    })
                    .cloned()
            })
        ...
        if !selection.output_args.json_mode {
            println!("=== Wait Strategy Myelon MMAP Benchmark ===");
            println!("Event size: {} bytes", ELEMENT_SIZE);
            println!("Buffer size: {} slots", MYELON_BENCH_BUFFER_SIZE);
