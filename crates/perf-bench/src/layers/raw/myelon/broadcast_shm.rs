...
use myelon::{
    attach_shared_consumer, build_shared_single_producer, AutoWaitStrategy, SharedConsumer,
...
fn message_consumer_sized<const SIZE: usize>() -> Result<(), Box<dyn std::error::Error>> {
    ...
    while warmup < warmup_target {
        ...
        if warmup == before {
            infra::check_deadline(warmup_deadline, "raw_myelon_shm message_consumer warmup");
            std::hint::spin_loop();
        }
    }
    ...
    loop {
        ...
        if consumed == before {
            infra::check_deadline(measure_deadline, "raw_myelon_shm message_consumer measured");
            std::hint::spin_loop();
        }
    }
    let elapsed = start.elapsed();
...
fn signal_consumer() -> Result<(), Box<dyn std::error::Error>> {
    ...
    while warmup < warmup_target {
        ...
        if warmup == before {
            infra::check_deadline(warmup_deadline, "raw_myelon_shm signal_consumer warmup");
            std::hint::spin_loop();
        }
    }
    ...
    loop {
        ...
        if consumed == before {
            infra::check_deadline(measure_deadline, "raw_myelon_shm signal_consumer measured");
            std::hint::spin_loop();
        }
    }
    let elapsed = start.elapsed();
...
fn multi_signal_consumer() -> Result<(), Box<dyn std::error::Error>> {
    ...
    loop {
        ...
        if warmup_count + consumed == before_progress {
            infra::check_deadline(
                warmup_deadline,
                "raw_myelon_shm multi_signal_consumer warmup",
            );
            std::hint::spin_loop();
        }
    }
    ...
    while consumed < events {
        ...
        if consumed == before {
            infra::check_deadline(
                measure_deadline,
                "raw_myelon_shm multi_signal_consumer measured",
            );
            std::hint::spin_loop();
        }
    }
...
fn multi_message_consumer_sized<const SIZE: usize>() -> Result<(), Box<dyn std::error::Error>> {
    ...
    loop {
        ...
        if warmup_count + consumed == before_progress {
            infra::check_deadline(
                warmup_deadline,
                "raw_myelon_shm multi_message_consumer warmup",
            );
            std::hint::spin_loop();
        }
    }
    ...
    while consumed < events {
        ...
        if consumed == before {
            infra::check_deadline(
                measure_deadline,
                "raw_myelon_shm multi_message_consumer measured",
            );
            std::hint::spin_loop();
        }
    }
...
const CHILD_ROLES: &[infra::ChildRole] = &[
    infra::ChildRole::new("my_msg_producer", message_producer),
    infra::ChildRole::new("my_msg_consumer", message_consumer),
    infra::ChildRole::new("my_sig_producer", signal_producer),
    infra::ChildRole::new("my_sig_consumer", signal_consumer),
    infra::ChildRole::new("my_sig_multi_producer", multi_signal_producer),
    infra::ChildRole::new("my_sig_multi_consumer", multi_signal_consumer),
    infra::ChildRole::new("my_msg_multi_producer", multi_message_producer),
    infra::ChildRole::new("my_msg_multi_consumer", multi_message_consumer),
];

pub struct RawMyelonShmBench;

impl infra::BenchHarness for RawMyelonShmBench {
    ...
    fn run_orchestrator(&self, args: &[String]) -> infra::BenchRunResult {
        ...
        if !selection.output_args.json_mode {
            println!("=== Raw Myelon SHM Benchmark ===");
            println!("Backend: POSIX shared memory (via myelon re-exports)");
            println!(
                "Mode: {}",
                if target_rate > 0 {
                    "co_aware"
