...
fn spawn_echo_process(
    ...
    child_cmd
        .arg("--process-two")
        .env(crate::infra::env::MMAP_ROOT, root)
...
fn run_benchmark<const SIZE: usize>(
    ...
    let root = infra::unique_mmap_root("pingpong_raw_myelon_mmap");
    ...
    for lane_idx in 0..args.consumers {
        let ping_segment = infra::unique_mmap_segment(&format!("cmp_ping_{lane_idx}"));
        let pong_segment = infra::unique_mmap_segment(&format!("cmp_pong_{lane_idx}"));
    ...
    if args.target_rate.is_none() {
        run_warmup(&args, &mut lanes, "pingpong_raw_myelon_mmap warmup receive")?;
    }
...
fn run_process_two() -> Result<(), Box<dyn std::error::Error>> {
    let root = PathBuf::from(env::var(crate::infra::env::MMAP_ROOT)?);
    ...
    while !coordination.is_shutdown() {
        match ping_consumer.try_consume_next_leased() {
            ...
            None => {
                ...
                infra::check_deadline(deadline, "pingpong_raw_myelon_mmap echo loop");
                pingpong::apply_wait_strategy(wait_strategy);
            }
        }
    }
