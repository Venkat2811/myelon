...
const CHILD_ROLES: &[infra::ChildRole] = &[
    infra::ChildRole::new("my_shm_wait_producer", producer_process),
    infra::ChildRole::new("my_shm_wait_consumer", consumer_process),
];

pub struct WaitStrategyMyelonShmBench;

impl infra::BenchHarness for WaitStrategyMyelonShmBench {
    ...
    fn run_orchestrator(&self, args: &[String]) -> infra::BenchRunResult {
        ...
        if !selection.output_args.json_mode {
            println!("=== Wait Strategy Myelon SHM Benchmark ===");
            println!(
