//! `PingPong` raw-myelon benchmark over the SHM backend.
//!
...
use myelon::{
    attach_shared_consumer, build_shared_single_producer, producer::CoordinationMode,
...
fn build_report(
    ...
    let mut result = reporting::make_result(reporting::BenchResultSpec {
        bench_name: "pingpong_raw_myelon_shm".to_string(),
        scenario: format!(
        ...
        payload_bytes: pingpong::payload_bytes_for_message_size(args.message_size).unwrap_or(
            args.message_size
                .saturating_sub(pingpong::PINGPONG_HEADER_BYTES),
        ),
    ...
    while messages_processed < args.num_messages {
        ...
        for (lane_idx, expected, send_start) in pending {
            let response_sequence = wait_for_next_event(
                ...
                &args.wait_strategy,
                deadline,
                "pingpong_raw_myelon_shm throughput receive",
                |response| response.sequence,
            )?;
            assert_eq!(response_sequence, expected);
    ...
    while messages_processed < args.num_messages {
        ...
        for (lane_idx, expected, send_start) in pending {
            let response_sequence = wait_for_next_event(
                ...
                &args.wait_strategy,
                deadline,
                "pingpong_raw_myelon_shm batch timing receive",
                |response| response.sequence,
            )?;
            assert_eq!(response_sequence, expected);
            recorder.record(send_start.elapsed().as_nanos() as u64);
...
fn run_fixed_rate_mode<const SIZE: usize>(
    ...
    run_warmup(
        args,
        lanes,
        "pingpong_raw_myelon_shm fixed-rate warmup receive",
    )?;
        ...
        for (lane_idx, expected) in pending {
            let (response_sequence, response_timestamp_ns, response_intended_send_time_ns) =
                wait_for_next_event(
                    ...
                    &args.wait_strategy,
                    deadline,
                    "pingpong_raw_myelon_shm fixed-rate receive",
                    |response| {
                        (
                            response.sequence,
...
fn run_benchmark<const SIZE: usize>(
    ...
    if !pingpong::json_mode(&args) {
        println!("=== PingPong Raw Myelon SHM ===");
        println!("Message size: {}", pingpong::human_size(args.message_size));
    ...
    if args.target_rate.is_none() {
        run_warmup(&args, &mut lanes, "pingpong_raw_myelon_shm warmup receive")?;
    }
...
fn run_process_one(args: Args) -> Result<(), Box<dyn std::error::Error>> {
    ...
    pingpong::run_with_large_stack_if_needed(
        args.message_size,
        "pingpong-raw-myelon-shm-process-one",
        move || {
...
fn run_process_two() -> Result<(), Box<dyn std::error::Error>> {
    ...
    pingpong::run_with_large_stack_if_needed(
        message_size,
        "pingpong-raw-myelon-shm-process-two",
        move || {
    ...
    while !coordination.is_shutdown() {
        match ping_consumer.try_consume_next_leased() {
            ...
            None => {
                ...
                infra::check_deadline(deadline, "pingpong_raw_myelon_shm echo loop");
                pingpong::apply_wait_strategy(wait_strategy);
            }
        }
    }
...
pub fn run_main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let filtered_args = infra::apply_timeout_arg(&filtered_args)
        .map_err(|error| format!("pingpong_raw_myelon_shm failed: {error}"))?;
...
pub fn run_main_with_args(args: Vec<String>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let filtered_args = infra::apply_timeout_arg(&args)
        .map_err(|error| format!("pingpong_raw_myelon_shm failed: {error}"))?;
