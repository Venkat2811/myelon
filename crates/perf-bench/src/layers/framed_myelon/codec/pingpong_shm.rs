...
impl IpcBenchmark for Scenario {
    ...
    fn launch(&self, exe: &std::path::Path) -> Result<ScenarioChildren, infra::BenchError> {
        ...
        let env_common = vec![
            (crate::infra::env::PING_SEGMENT, ping_segment),
            (crate::infra::env::PONG_SEGMENT, pong_segment),
            (
...
const CHILD_ROLES: &[infra::ChildRole] = &[
    infra::ChildRole::new("pingpong_codec_shm_echo", producer_process),
    infra::ChildRole::new("pingpong_codec_shm_initiator", consumer_process),
];

pub struct PingPongCodecShmBench;

impl infra::BenchHarness for PingPongCodecShmBench {
