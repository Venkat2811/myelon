...
) -> Result<MmapTypedConsumer<ZcFrame>, Box<dyn std::error::Error>> {
...
fn producer_process() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let pong_layout = MmapTransportLayout::new(&env.root, &env.pong_segment)?;
    let ping_layout = MmapTransportLayout::new(&env.root, &env.ping_segment)?;
    let mut pong_producer =
        MmapTypedProducer::<ZcFrame>::create(pong_layout.clone(), env.buffer_depth)?;
    coordination
        .data()
        .producer_ready
        .store(1, Ordering::Release);

    let mut ping_consumer =
        attach_consumer_with_timeout(ping_layout, env.buffer_depth, PING_ECHO_ID, wait_strategy)?;
    coordination
        .data()
        .echo_attached
        .store(1, Ordering::Release);
    ...
    match env.codec.as_str() {
        "rkyv" => {
            ...
            for index in 0..total {
                ...
                pong_producer.publish(&response, kind)?;
            }
        }
        "flatbuf" => {
...
fn consumer_process() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut ping_producer =
        MmapTypedProducer::<ZcFrame>::create(ping_layout.clone(), env.buffer_depth)?;
...
impl IpcBenchmark for Scenario {
    ...
    fn launch(&self, exe: &std::path::Path) -> Result<ScenarioChildren, infra::BenchError> {
        let root = unique_mmap_root("mpzc_mmap");
        ...
        let env_common = vec![
            ...
            (
                crate::infra::env::ENCODED_BYTES,
                self.encoded_bytes.to_string(),
            ),
...
const CHILD_ROLES: &[infra::ChildRole] = &[
    infra::ChildRole::new("pingpong_typed_zero_copy_mmap_echo", producer_process),
    infra::ChildRole::new("pingpong_typed_zero_copy_mmap_initiator", consumer_process),
];

pub struct PingPongTypedZeroCopyMmapBench;

impl infra::BenchHarness for PingPongTypedZeroCopyMmapBench {
    ...
    fn run_orchestrator(&self, args: &[String]) -> infra::BenchRunResult {
        ...
        if !selection.output_args.json_mode {
            println!("=== Myelon Typed Zero-Copy MMAP Ping-Pong ===");
            println!("Mode: {}", selection.mode.description());
            if selection.target_rate > 0 {
                println!("Target rate: {} msgs/s", selection.target_rate);
            }
            println!("Transport: TypedTransport zero-copy receive over mmap, strict 1p1c");
        ...
        for spec in selection.scenario_specs_zero_copy(BackendKind::Mmap) {
