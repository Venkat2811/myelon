...
fn read_env() -> (disruptor_mp::MmapTransportLayout, usize, u64, usize) {
    ...
    let payload_bytes: usize = env::var(crate::infra::env::PAYLOAD_BYTES)
        .expect(crate::infra::env::PAYLOAD_BYTES)
        .parse()
        .expect("payload bytes");
    let messages: u64 = env::var(crate::infra::env::MESSAGES)
...
impl IpcBenchmark for Scenario {
    ...
    fn launch(&self, exe: &std::path::Path) -> Result<ScenarioChildren, infra::BenchError> {
        ...
        let env_common: Vec<(&str, String)> = vec![
            (crate::infra::env::MMAP_ROOT, root_str),
...
const CHILD_ROLES: &[infra::ChildRole] = &[
    infra::ChildRole::new("framed_mmap_producer", producer_process),
    infra::ChildRole::new("framed_mmap_consumer", consumer_process),
];

pub struct FramedMmapBench;

impl infra::BenchHarness for FramedMmapBench {
