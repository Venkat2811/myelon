...
fn child_layout() -> MmapTransportLayout {
    infra::mmap_layout_from_env("SWEEP_ROOT", "SWEEP_SEGMENT")
}
...
fn signal_producer() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let consumers = infra::read_env_usize("SWEEP_CONSUMERS", 1);
...
fn signal_consumer() -> Result<(), Box<dyn std::error::Error>> {
    ...
    while consumed < events {
        if let Some(event) = consumer.try_consume_next_leased() {
            ...
            infra::check_deadline(deadline, "monster_sweep_mmap signal_consumer measured");
            std::hint::spin_loop();
        }
    }
    let elapsed = start.elapsed();
...
macro_rules! sweep_impl {
    ($ev_type:ty, $prod_fn:ident, $cons_fn:ident) => {
        fn $prod_fn() -> Result<(), Box<dyn std::error::Error>> {
            ...
            let consumers = infra::read_env_usize("SWEEP_CONSUMERS", 1);
            let target_rate = infra::read_env_u64("SWEEP_TARGET_RATE", 0);
...
impl IpcBenchmark for SweepPoint {
    ...
    fn launch(&self, exe: &std::path::Path) -> Result<ScenarioChildren, infra::BenchError> {
        let root = infra::unique_mmap_root(&self.tag);
        let segment = infra::unique_mmap_segment(&self.tag);
        ...
        let consumers = (0..self.consumers)
            .map(|i| {
                let mut consumer_env = vec![
                    ...
                    (crate::infra::env::CONSUMER_ID, i.to_string()),
                ];
                if self.target_rate > 0 {
                    consumer_env.push(("SWEEP_TARGET_RATE", self.target_rate.to_string()));
                }
                infra::spawn_child(exe, self.cons_role, &consumer_env)
            })
            .collect();
...
impl infra::BenchHarness for MonsterSweepMmap {
