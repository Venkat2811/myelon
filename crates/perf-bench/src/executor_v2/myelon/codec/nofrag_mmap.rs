...
macro_rules! impl_nofrag_bench {
    ($slot_type:ty, $slot_data_len:expr, $producer_fn:ident, $consumer_fn:ident) => {
        fn $producer_fn() -> Result<(), Box<dyn std::error::Error>> {
            let layout = harness::mmap_layout_from_env("BENCH_MMAP_ROOT", "BENCH_MMAP_SEGMENT");
            ...
            let mut producer =
                MmapProducer::<$slot_type>::create(layout, buffer_depth, || <$slot_type>::default())?;
...
impl harness::BenchHarness for CodecNoFragMmapBench {
