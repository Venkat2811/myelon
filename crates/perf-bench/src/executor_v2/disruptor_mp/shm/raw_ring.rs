...
fn message_consumer() -> Result<(), Box<dyn std::error::Error>> {
    ...
    while warmup < WARMUP {
        ...
        if warmup == before {
            harness::check_deadline(warmup_deadline, "raw_ring_shm message_consumer warmup");
            std::hint::spin_loop();
        }
    }
    ...
    loop {
        ...
        if consumed == before {
            harness::check_deadline(measure_deadline, "raw_ring_shm message_consumer measured");
            std::hint::spin_loop();
        }
    }
    let elapsed = start.elapsed();
...
fn signal_consumer() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let consumer_id = harness::read_env_usize("BENCH_CONSUMER_ID", 0);
    let buffer = harness::read_env_usize("BENCH_BUFFER", 65_536);
    let warmup_target = harness::read_env_u64("BENCH_WARMUP", 100_000);
    ...
    while warmup < warmup_target {
        ...
        if warmup == before {
            harness::check_deadline(warmup_deadline, "raw_ring_shm signal_consumer warmup");
            std::hint::spin_loop();
        }
    }
    ...
    loop {
        ...
        if consumed == before {
            harness::check_deadline(measure_deadline, "raw_ring_shm signal_consumer measured");
            std::hint::spin_loop();
        }
    }
    let elapsed = start.elapsed();
