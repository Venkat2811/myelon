...
fn typed_zc_shm_cons() -> Result<(), Box<dyn std::error::Error>> {
    ...
    while consumed < num_messages {
        ...
        if consumed < num_messages {
            harness::check_deadline(deadline, "typed_zc_shm_cons measured");
            if delivered == 0 {
                std::hint::spin_loop();
            }
        }
...
fn typed_zc_mmap_cons() -> Result<(), Box<dyn std::error::Error>> {
    ...
    while consumed < num_messages {
        ...
        if consumed < num_messages {
            harness::check_deadline(deadline, "typed_zc_mmap_cons measured");
            if delivered == 0 {
                std::hint::spin_loop();
            }
        }
...
const CHILD_ROLES: &[harness::ChildRole] = &[
    harness::ChildRole::new("typed_zc_shm_prod", typed_zc_shm_prod),
    harness::ChildRole::new("typed_zc_shm_cons", typed_zc_shm_cons),
    harness::ChildRole::new("typed_zc_mmap_prod", typed_zc_mmap_prod),
    harness::ChildRole::new("typed_zc_mmap_cons", typed_zc_mmap_cons),
];

pub struct TypedZeroCopySweep;

impl harness::BenchHarness for TypedZeroCopySweep {
