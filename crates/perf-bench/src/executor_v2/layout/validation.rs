...
pub fn run_main() {
    ...
    let args = match crate::harness::apply_timeout_arg(&raw_args) {
        Ok(args) => args,
        Err(error) => {
    ...
    let _log = crate::bench_log::BenchLog::default_capacity("layout_validation");
    ...
    let report =
        report_v2::ReportBundle::from_layout_targets("layout_validation", ITERATIONS, &targets);
    report_v2::emit_report(
        &report,
        &output_args,
