        ...
        .map(|scenario| {
            ...
            Row {
                ...
                alloc_bytes: outcome
                    ...
                    .unwrap_or_else(|| "-".to_string()),
                prod_ops: crate::infra::events::format_throughput(
                    outcome.producer.throughput_ops_sec,
                ),
                cons_ops: crate::infra::events::format_throughput(
                    outcome.consumers.average_throughput_ops_sec,
                ),
                p50: outcome
                    .latency
                    .as_ref()
                    .map(|stats| crate::infra::latency::format_ns(stats.p50_ns))
                    .unwrap_or_else(|| "-".to_string()),
                p99: outcome
                    .latency
                    .as_ref()
...
fn format_avg_ns(value: f64) -> String {
    if value < 1_000.0 {
        ...
        crate::infra::latency::format_ns(value.round() as u64)
    }
}
