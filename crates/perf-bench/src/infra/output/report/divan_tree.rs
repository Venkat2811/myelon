...
impl TreePainter {
    ...
    fn write_leaf(&mut self, name: &str, is_last: bool, primary: &[String], secondary: &[String]) {
    ...
    pub fn render_tree(&self) -> String {
        ...
        render_grouped_tree(
            ...
            self.scenarios
                .iter()
                .map(|scenario| {
                    let lat = |f: fn(&crate::infra::latency::LatencyStats) -> u64| match &scenario
                        .outcome
                    {
                    ...
                    let p1 = lat(|s| s.p1_ns);
                    let p10 = lat(|s| s.p10_ns);
                    let p25 = lat(|s| s.p25_ns);
                    let p50 = lat(|s| s.p50_ns);
                    let p90 = lat(|s| s.p90_ns);
                    let p99 = lat(|s| s.p99_ns);
                    let p999 = lat(|s| s.p999_ns);
                    let p9999 = lat(|s| s.p9999_ns);
                    let p999999 = lat(|s| s.p999999_ns);
                    ...
                    TreeLeaf {
                        ...
                        primary_columns: vec![
                            payload, depth, consumers, prod_ops, cons_ops, prod_bw, cons_bw, p1,
                            p10, p25, p50, p90, p99, p999, p9999, p999999, pct_raw, delta_raw,
                        ],
...
mod tests {
    ...
    use crate::infra::output::report::{LayoutTargetMeasurement, ReportBundle, ReportBundleCompat};
    ...
    #[test]
    fn renders_layout_tree_from_report_bundle() {
    ...
    fn renders_multi_line_tree_with_secondary_row() {
        ...
        report.add(result);
        let tree = report.to_report().render_tree();

        // Verify primary line has measurement data
        assert!(
            tree.contains("10.00M"),
            "should contain producer throughput"
        );
