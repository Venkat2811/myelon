...
pub trait IpcBenchmark {
    ...
    fn run_benchmark(&self) -> Result<BenchResult, BenchError> {
        ...
        std::env::set_var(crate::infra::env::TIMEOUT, timeout.as_secs().to_string());
...
pub trait BenchHarness {
    ...
    fn run(&self) {
        ...
        let args = match super::config::apply_timeout_arg(&raw_args) {
            Ok(args) => args,
            Err(error) => {
