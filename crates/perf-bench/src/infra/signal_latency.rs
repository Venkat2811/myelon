...
impl SignalLatencyMode {
    ...
    pub fn from_env() -> Result<Self, String> {
        let raw =
            std::env::var("PERF_BENCH_SIGNAL_LATENCY_MODE").unwrap_or_else(|_| "none".to_string());
        Self::parse(&raw)
    }
...
pub fn sample_every_from_env() -> u64 {
    std::env::var("PERF_BENCH_SIGNAL_SAMPLE_EVERY")
        .ok()
...
pub fn shm_sidecar_name_from_env() -> Result<String, Box<dyn std::error::Error>> {
    Ok(std::env::var("PERF_BENCH_SIGNAL_SIDECAR_SHM_ID")?)
}
...
mod tests {
    ...
    fn sample_every_zero_defaults_to_one() {
        std::env::set_var("PERF_BENCH_SIGNAL_SAMPLE_EVERY", "0");
        assert_eq!(sample_every_from_env(), 1);
        std::env::remove_var("PERF_BENCH_SIGNAL_SAMPLE_EVERY");
    }
