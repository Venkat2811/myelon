...
fn run_raw_myelon(args: &Args) -> Result<(), Box<dyn Error>> {
    ...
    match args.backend.as_str() {
        "shm" => {
            std::env::set_var(perf_bench::infra::env::BROADCAST_HARNESS, "raw_myelon_shm");
            let bench = perf_bench::layers::raw::myelon::broadcast_shm::RawMyelonShmBench;
            bench.run_orchestrator(&synthetic)?;
            Ok(())
        }
        "mmap" => {
            std::env::set_var(perf_bench::infra::env::BROADCAST_HARNESS, "raw_myelon_mmap");
...
fn run_framed(args: &Args) -> Result<(), Box<dyn Error>> {
    ...
    match args.backend.as_str() {
        "shm" => {
            std::env::set_var(perf_bench::infra::env::BROADCAST_HARNESS, "framed_shm");
            let bench = perf_bench::layers::framed_myelon::frag::broadcast_shm::FramedShmBench;
            bench.run_orchestrator(&synthetic)?;
            Ok(())
        }
        "mmap" => {
            std::env::set_var(perf_bench::infra::env::BROADCAST_HARNESS, "framed_mmap");
...
fn run_codec(args: &Args) -> Result<(), Box<dyn Error>> {
    ...
    match args.backend.as_str() {
        "shm" => {
            std::env::set_var(perf_bench::infra::env::BROADCAST_HARNESS, "codec_shm");
            let bench = perf_bench::layers::framed_myelon::codec::shm::CodecE2eShmBench;
            bench.run_orchestrator(&synthetic)?;
            Ok(())
        }
        "mmap" => {
            std::env::set_var(perf_bench::infra::env::BROADCAST_HARNESS, "codec_mmap");
