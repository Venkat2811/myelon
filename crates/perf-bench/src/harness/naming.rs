...
pub fn unique_shm_segment(prefix: &str) -> String {
    ...
    disruptor_mp::portable_shm_segment_name(&format!(
        "{}_{}_{}", prefix, std::process::id() % 10000, ts % 100000
    ))
}
...
pub fn mmap_layout_from_env(root_key: &str, segment_key: &str) -> disruptor_mp::MmapTransportLayout {
    ...
    disruptor_mp::MmapTransportLayout::new(PathBuf::from(root), segment)
        .expect("mmap layout")
}
