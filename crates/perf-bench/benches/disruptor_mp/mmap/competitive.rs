...
struct Args {
    ...
    /// Keep CLI parity with competitive_shm; mmap has no external competitor data
    #[arg(long)]
    no_compare: bool,
...
fn print_header(args: &Args, buffer_size: usize) {
    println!("=== Competitive MMAP Ping-Pong ===");
...
fn run_throughput_mode<const SIZE: usize>(
    args: &Args,
    producer_ping: &mut MmapProducer<BenchmarkEvent<SIZE>>,
    pong_consumer: &mut MmapConsumer<BenchmarkEvent<SIZE>>,
...
fn run_fixed_rate_mode<const SIZE: usize>(
    ...
    for i in 0..args.warmup {
        ...
        let (_sequence, response) = wait_for_next_event(
            ...
            &args.wait_strategy,
            deadline,
            "competitive_mmap fixed-rate warmup receive",
        )?;
...
fn run_benchmark<const SIZE: usize>(
    ...
    let ping_segment = harness::unique_mmap_segment("cmp_ping");
    let pong_segment = harness::unique_mmap_segment("cmp_pong");
    let coordination_name = portable_shm_segment_name("cmpmmap");
    ...
    let mut producer_ping = MmapProducer::<BenchmarkEvent<SIZE>>::create(
        ping_layout,
        buffer_size,
    ...
    if !producer_ping.wait_for_consumers_ready(1, timeout) {
        return Err("timeout waiting for ping consumer readiness".into());
    }
    ...
    for i in 0..args.warmup {
        ...
        let (_sequence, response) = wait_for_next_event(
            ...
            "competitive_mmap warmup receive",
        )?;
        assert_eq!(response.sequence, i);
        coordination
            .data()
