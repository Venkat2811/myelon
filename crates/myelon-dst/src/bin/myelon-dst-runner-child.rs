//! Deterministic-simulation runner child binary.
//!
//! Compiles to a stub that exits non-zero when built without
//! `RUSTFLAGS="--cfg dst"`. The real implementation lives in `mod inner`
//! which is gated by `#[cfg(dst)]`. This shape lets `--all-features`
//! activate the `required-features = ["_runner_bin"]` flag in Cargo.toml
//! without the file becoming main-less.

#[cfg(not(dst))]
fn main() {
    eprintln!(
        "myelon-dst-runner-child requires RUSTFLAGS=\"--cfg dst\". See crates/myelon-dst/README.md."
    );
    std::process::exit(2);
}
...
mod inner {
    ...
    fn dst_discovery_poll_duration() -> Duration {
        let default_ms = u64::try_from(disruptor_mp::default_discovery_poll_duration().as_millis())
            .unwrap_or(10);
    ...
    fn run_shm_producer() -> ChildReport {
        let segment = env::var(dst_env::SEGMENT)
            .unwrap_or_else(|_| panic!("{} should be set", dst_env::SEGMENT));
        ...
        let corrupt_at_sequence: Option<u64> =
            env::var(dst_env::CORRUPT_AT_SEQUENCE).ok().map(|raw| {
                raw.parse::<u64>()
